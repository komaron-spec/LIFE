...
const skillState = () => (state.skills ||= { selected:"human-basics", zoom:.72 });
...
function renderPage(page) {
  ...
  const now = new Date();
  applyWorldAtmosphere(w);
  let content = page === "skills" ? renderSkillsPage() : "";
  ...
  app.querySelectorAll("[data-skill-node]").forEach((button) => button.addEventListener("click", () => { skillState().selected = button.dataset.skillNode; save(); renderPage("skills"); }));
