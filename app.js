...
function renderHome() {
  ...
  if (page === "skills") localiseSkillChrome();
...
function renderPage(page) {
  ...
  app.querySelector("#save-skill-record")?.addEventListener("click", () => { const skills = skillState(); const node = skillNodes.find((item) => item.id === skills.selected); if (!node) return; const previous = skillStatus(node); const status = app.querySelector("#skill-status-select").value; const note = app.querySelector("#skill-note").value.trim(); skills.records[node.id] = { status, note, updatedAt:new Date().toISOString() }; skills.history.unshift({ id:crypto.randomUUID(), nodeId:node.id, label:node.label, status, note, date:new Date().toLocaleDateString("en-CA") }); skills.history = skills.history.slice(0,200); save(); systemFeedback(status === "locked" ? "lock" : "discovery"); skills.mode = "history"; renderPage("skills"); });
