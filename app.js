...
async function loadCalendarEvents(accessToken) {
  const now = new Date(); const until = new Date(now); until.setDate(until.getDate() + 7);
  const params = new URLSearchParams({ timeMin:now.toISOString(), timeMax:until.toISOString(), singleEvents:"true", orderBy:"startTime", maxResults:"12" });
  ...
  state.log.unshift({ id:crypto.randomUUID(), day:dateKey(), time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}), kind:"SYSTEM", title:"CALENDAR SYNC COMPLETE", detail:`${state.calendar.events.length} upcoming events loaded.` });
  save();
}
