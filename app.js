...
const fieldScanState = () => (state.fieldScans ||= { areas:{} });
let systemAudio;
let idleTimer;
...
function renderFieldScanCollection() {
  const entries = Object.values(fieldScanState().areas).sort((a,b) => new Date(b.lastScannedAt) - new Date(a.lastScannedAt));
  return `<section class="detail-card glass field-scan-codex"><div class="section-head"><p class="eyebrow">visual field codex</p><span>${entries.length} AREAS</span></div><strong>観測済みエリア</strong><p>カメラを向けてFIELD SCANした場所だけが、この図鑑に残ります。</p><div>${entries.length ? entries.slice(0,12).map((entry) => `<article><i>◌</i><span><b>${esc(entry.location)}</b><small>${esc(entry.region)} · ${entry.scans} SCANS</small></span><time>${new Date(entry.lastScannedAt).toLocaleDateString("ja-JP")}</time></article>`).join("") : `<p class="scan-codex-empty">まだ観測済みのエリアはありません。WORLD VIEWからカメラでFIELD SCANしてください。</p>`}</div></section>`;
}
...
function scanCurrentField() {
  if (!worldCameraStream) return;
  const portal = app.querySelector(".world-camera-portal");
  const button = app.querySelector("#scan-current-field");
  if (button) { button.disabled = true; button.textContent = "SCANNING..."; }
  portal?.classList.add("is-scanning");
  systemFeedback("scan");
  setTimeout(() => {
    const world = state.world || {};
    const latitude = Number(world.latitude); const longitude = Number(world.longitude);
    const grid = Number.isFinite(latitude) && Number.isFinite(longitude) ? `${latitude.toFixed(3)}:${longitude.toFixed(3)}` : String(world.location || "unknown");
    const scans = fieldScanState(); const firstScan = !scans.areas[grid]; const now = new Date();
    scans.areas[grid] ||= { id:grid, location:world.location || "WORLD", region:world.region || "UNKNOWN REGION", firstScannedAt:now.toISOString(), scans:0 };
    scans.areas[grid].scans += 1; scans.areas[grid].lastScannedAt = now.toISOString();
    const title = firstScan ? "PLACE DISCOVERY" : "AREA SCANNED";
    const detail = firstScan ? `${scans.areas[grid].location} / visual field registered` : `${scans.areas[grid].location} / scan ${scans.areas[grid].scans}`;
    state.log.unshift({ id:crypto.randomUUID(), day:dateKey(), time:now.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}), kind:"SCAN", title, detail });
    recordSystemMessage({ level:firstScan ? 2 : 1, title, detail, target:"archive" });
    save(); systemFeedback(firstScan ? "discovery" : "confirm");
    portal?.classList.remove("is-scanning"); portal?.classList.add("scan-complete");
    if (button) { button.disabled = false; button.textContent = firstScan ? "DISCOVERY REGISTERED" : "SCAN COMPLETE"; }
    const stateLabel = app.querySelector("#world-camera-state");
    if (stateLabel) stateLabel.textContent = firstScan ? "CODEX UPDATED" : "SCAN COMPLETE";
  }, 680);
}
...
function renderPage(page) {
  ...
  if (page === "world") content = `<section class="page-hero world-page-hero world-scan-hero"><p class="eyebrow">world view / live</p><h1>WORLD VIEW</h1><p>端末を現実へ向けて、いまいる世界を観測する。</p><div class="world-orbital"><span class="location-mark">${icon("location")}</span><div><strong>${esc(w.location)}</strong><small>${esc(w.region)}</small></div><b>${esc(w.temperature)}°</b></div></section><section class="detail-card glass world-camera-card"><div class="section-head"><p class="eyebrow">world view</p><span id="world-camera-state">CAMERA OFF</span></div><strong>現実世界をスキャンする</strong><p>カメラ映像と現在地を照合し、観測した領域をPLAYER LOGとCODEXへ残します。映像は端末外へ送信・保存しません。</p><div class="world-camera-portal"><video id="world-camera-video" autoplay playsinline muted></video><div class="world-camera-overlay"><span>LIVE WORLD VIEW</span><b>${esc(w.location || "WORLD")}</b><i>● LINK STABLE</i><u></u><button id="scan-current-field">SCAN CURRENT FIELD</button></div></div><button class="primary-action" id="open-world-camera">カメラを起動する</button><button class="subtle-action" id="close-world-camera">カメラを閉じる</button></section><section class="detail-card glass"><p class="eyebrow">world conditions</p><div class="condition-list"><div><span>${icon(phaseIcon(w.phase))}</span><p>TIME<b>${esc(w.phase)}</b></p></div><div><span>${icon(weatherIcon(w.weather))}</span><p>WEATHER<b>${esc(w.weather)}</b></p></div><div><span>${icon("sun")}</span><p>SEASON<b>${esc(w.season)}</b></p></div></div><p class="ambience">${esc(w.ambience)}</p><button class="primary-action" id="page-sync">SYNC WORLD</button></section>`;
  ...
  if (page === "archive") content += renderAtlasProgressPanel(w) + renderFieldScanCollection();
  ...
  app.querySelector("#scan-current-field")?.addEventListener("click", scanCurrentField);
