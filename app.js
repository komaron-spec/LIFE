...
function renderSoundNowPlaying(selected, suggested) {
  const mode = state.soundtrackId ? "PLAYER SELECTED" : "WORLD AUTO";
  return `<section class="now-playing-screen glass ${selected.tone}"><div class="now-playing-head"><p class="eyebrow">now playing</p><span><i></i>${mode}</span></div><div class="now-playing-orb" aria-hidden="true"><b></b><i></i><i></i><i></i><em></em></div><div class="now-playing-copy"><small>${selected.id === suggested.id ? "CURRENT WORLD SCORE" : "PLAYER OVERRIDE"}</small><h2>${esc(selected.scene)}</h2><p>${esc(selected.role)}</p></div><div class="now-playing-wave" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div><iframe title="${esc(selected.scene)} Spotify player" src="https://open.spotify.com/embed/track/${selected.track}?utm_source=generator" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe><p class="now-playing-note">Spotifyで再生すると、このWORLD SCENEを現実へ重ねられます。</p></section>`;
}
...
function renderPage(page, transitionDirection = "forward") {
  ...
  if (page === "sound") { const suggested = soundtrackFor(w); const selected = soundtrackLibrary.find((track) => track.id === state.soundtrackId) || suggested; content = content.replace('<section class="sound-player', `${renderSoundNowPlaying(selected, suggested)}<section class="sound-player`); }
