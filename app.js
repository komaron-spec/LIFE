...
function renderPage(page) {
  ...
  if (page === "player") { const profile = playerProfile(); content = `<section class="page-hero player-hero"><p class="eyebrow">character profile</p><h1>PLAYER</h1><p>現実を歩く、あなた自身のステータス。</p><div class="player-portrait"><div class="player-level-ring" style="--level-progress:${state.player.exp / 5}%"><div class="level glass"><span class="eyebrow">lv</span><strong>${state.player.level}</strong></div></div><div><strong>${esc(profile.name)}</strong><small>${profile.title}<br>DAY START · ${esc(state.status?.[dateKey()] || "UNKNOWN")}</small></div></div></section><section class="detail-card glass player-identity"><div class="section-head"><p class="eyebrow">player identity</p><span>LOCAL PROFILE</span></div><input id="player-name" maxlength="24" value="${esc(profile.name)}" aria-label="プレイヤー名"><button class="subtle-action" id="save-player-name">SAVE IDENTITY</button></section><section class="detail-card glass"><p class="eyebrow">current parameters</p><div class="parameter-list">${[["HP",state.player.hp,"#f4b9cc"],["ENERGY",state.player.energy,"#bfe5d0"],["FOCUS",state.player.focus,"#b8e5f2"],["EXP",state.player.exp,"#d8ccff"]].map(([label,value,color]) => `<div><p><span>${label}</span><b>${value}${label === "EXP" ? " / 500" : ""}</b></p><i><span style="width:${label === "EXP" ? value / 5 : value}%;background:${color}"></span></i></div>`).join("")}</div></section><section class="detail-card glass"><p class="eyebrow">status effects</p><div class="effect-list">${profile.effects.map(([name,copy,tone]) => `<div class="effect ${tone}"><i></i><div><strong>${name}</strong><p>${copy}</p></div></div>`).join("")}</div></section><section class="detail-card glass"><p class="eyebrow">abilities</p><div class="ability-list">${profile.abilities.map(([name,value,copy]) => `<div><p><span>${name}</span><b>${value}</b></p><i><span style="width:${value}%"></span></i><small>${copy}</small></div>`).join("")}</div></section>`; }
