...
function renderPage(page) {
  ...
  if (page === "system") { const calendar = state.calendar; const archive = archiveState(); content = `<section class="page-hero"><p class="eyebrow">life system settings</p><h1>SYSTEM</h1><p>現実とLIFE SYSTEMをつなぐ設定。</p><div class="system-status"><i></i><span>WORLD INTERFACE<br><b>ONLINE</b></span></div></section><section class="detail-card glass calendar-link"><div class="section-head"><p class="eyebrow">google calendar</p><span>${calendar ? "LINKED" : "NOT CONNECTED"}</span></div><strong>${calendar ? "WORLD SCHEDULE READY" : "CONNECT YOUR SCHEDULE"}</strong><p>${calendar ? `${calendar.calendars?.length || 1} calendars · ${calendar.events.length} events · ${new Date(calendar.syncedAt).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})} synced` : "すべてのGoogleカレンダーを読み取り、PLAYER LOGとWORLD MISSIONとして表示します。"}</p><button class="primary-action" id="connect-calendar">${calendar ? "REFRESH ALL CALENDARS" : "CONNECT GOOGLE CALENDAR"}</button></section><section class="detail-card glass save-data"><div class="section-head"><p class="eyebrow">save data</p><span>LOCAL BACKUP</span></div><strong>あなたの世界を保存する</strong><p>ATLASの攻略、PLAYER、設定を1つのセーブファイルに書き出します。都道府県 ${Object.keys(archive.prefectures).length}件・国 ${Object.keys(archive.countries).length}件を保存対象にしています。</p><div><button class="subtle-action" id="export-save">SAVE DATAを書き出す</button><button class="subtle-action" id="import-save">SAVE DATAを復元する</button><input id="import-save-file" type="file" accept="application/json,.json" hidden></div></section><section class="detail-card glass system-list"><button id="system-sync"><span>WORLD SYNC</span><small>現在地・天気を更新</small><b>→</b></button><button id="system-status"><span>DAY START STATUS</span><small>今日の状態を選び直す</small><b>→</b></button><div><span>PLAYER LOG</span><small>ALL GOOGLE CALENDARS · 手入力なし</small></div></section>`; }
