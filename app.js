...
function normalizeHomeAreaRadii() {
  let changed = false;
  areaState().forEach((area) => { if (area.type === "home" && Number(area.radius) === 180) { area.radius = 80; changed = true; } });
  if (changed) save();
}
normalizeHomeAreaRadii();
...
const homeBgmSettings = () => {
  const settings = state.homeBgm ||= { enabled:true, manualSceneId:null };
  if (!("manualSceneId" in settings)) settings.manualSceneId = null;
  return settings;
};
...
function selectedHomeBgmScene(now = new Date(), world = state.world || {}) {
  const manualScene = homeBgmScenes.find((scene) => scene.id === homeBgmSettings().manualSceneId);
  return manualScene || homeBgmScenes.find((scene) => scene.matches(now, world)) || homeBgmScenes.at(-1);
}
...
function bindLocalBgmPlayer() {
  ...
  const update = () => {
    const scene = selectedHomeBgmScene();
    if (audio !== homeBgm) audio = homeBgm;
    const active = Boolean(homeBgmSettings().enabled && audio && !audio.paused);
    const total = Number.isFinite(audio?.duration) ? audio.duration : 0;
    progress.max = String(Math.max(total, 1));
    progress.value = String(Math.min(Number(audio?.currentTime) || 0, Math.max(total, 1)));
    ...
    wave?.classList.toggle("is-playing", active);
    title.textContent = localBgmTitle(scene);
    sceneLabel.textContent = `${scene.label} · LOCAL LOOP`;
  };
  const attach = () => {
    audio = getHomeBgm(selectedHomeBgmScene());
    ["loadedmetadata", "durationchange", "timeupdate", "play", "pause", "ended"].forEach((event) => audio.addEventListener(event, update));
    update();
  };
  toggle.addEventListener("click", async () => {
    if (homeBgmSettings().enabled && homeBgm && !homeBgm.paused) stopHomeBgm();
    else { homeBgmSettings().enabled = true; save(); await startHomeBgm(); }
    update();
  });
...
const brightStars = [
  ["Sirius",6.752,-16.716,-1.46],["Arcturus",14.261,19.182,-.05],["Vega",18.615,38.783,.03],["Capella",5.279,45.998,.08],["Rigel",5.243,-8.202,.13],["Procyon",7.655,5.225,.38],["Betelgeuse",5.919,7.407,.42],["Altair",19.846,8.868,.77],["Aldebaran",4.598,16.509,.85],["Spica",13.42,-11.161,.97],["Antares",16.49,-26.432,.96],["Pollux",7.755,28.026,1.14],["Fomalhaut",22.961,-29.622,1.16],["Deneb",20.69,45.28,1.25],["Regulus",10.139,11.967,1.35],["Castor",7.576,31.888,1.58],["Bellatrix",5.419,6.35,1.64],["Alnilam",5.603,-1.202,1.69],["Alnair",22.137,-46.961,1.74],["Mirfak",3.406,49.861,1.79],["Dubhe",11.062,61.751,1.79],["Alphecca",15.579,26.715,2.23],["Polaris",2.53,89.264,1.98]
];
function renderCelestialStars(now, latitude, longitude) {
  const julian = now.getTime() / 86400000 + 2440587.5; const days = julian - 2451545.0;
  const localSidereal = ((18.697374558 + 24.06570982441908 * days + longitude / 15) % 24 + 24) % 24;
  const lat = latitude * Math.PI / 180;
  return brightStars.map(([name, ra, dec, magnitude]) => {
    const hourAngle = (localSidereal - ra) * Math.PI / 12; const declination = dec * Math.PI / 180;
    const altitude = Math.asin(Math.sin(lat) * Math.sin(declination) + Math.cos(lat) * Math.cos(declination) * Math.cos(hourAngle)) * 180 / Math.PI;
    if (altitude < 3) return "";
    const azimuth = Math.atan2(-Math.sin(hourAngle), Math.tan(declination) * Math.cos(lat) - Math.sin(lat) * Math.cos(hourAngle));
    const horizonDistance = Math.max(0, Math.min(1, altitude / 82)); const x = 50 + Math.sin(azimuth) * (45 - horizonDistance * 12); const y = 73 - horizonDistance * 63;
    const size = Math.max(.7, Math.min(2.35, 2.45 - (magnitude + 1.5) * .56)); const opacity = Math.max(.34, Math.min(.98, 1 - (magnitude + 1.5) * .13));
    return `<i title="${name}" style="--x:${x.toFixed(2)}%;--y:${y.toFixed(2)}%;--size:${size.toFixed(2)}px;--brightness:${opacity.toFixed(2)}"></i>`;
  }).join("");
}
function blendHex(from, to, amount) { const a = from.match(/\w\w/g).map((value) => parseInt(value,16)); const b = to.match(/\w\w/g).map((value) => parseInt(value,16)); return `#${a.map((value,index) => Math.round(value + (b[index] - value) * amount).toString(16).padStart(2,"0")).join("")}`; }
function solarProjection(now = new Date(), latitude = 34.6937, longitude = 135.5023) {
  const day = Math.floor((Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()) - Date.UTC(now.getFullYear(),0,0)) / 86400000); const hour = now.getHours() + now.getMinutes() / 60 + now.getSeconds() / 3600;
  const gamma = 2 * Math.PI / 365 * (day - 1 + (hour - 12) / 24); const equation = 229.18 * (.000075 + .001868 * Math.cos(gamma) - .032077 * Math.sin(gamma) - .014615 * Math.cos(2 * gamma) - .040849 * Math.sin(2 * gamma));
  ...
  const zone = -now.getTimezoneOffset() / 60; const solarMinutes = hour * 60 + equation + 4 * longitude - 60 * zone; const angle = solarMinutes / 4 - 180; const radians = angle * Math.PI / 180; const lat = latitude * Math.PI / 180;
  const altitude = Math.asin(Math.sin(lat) * Math.sin(declination) + Math.cos(lat) * Math.cos(declination) * Math.cos(radians)) * 180 / Math.PI;
  const visibility = Math.max(0, Math.min(1, (altitude + 8) / 18)); const noon = Math.max(0, Math.min(1, altitude / 55)); const phase = moonIllumination(now); const illumination = Math.round((1 - Math.cos(phase * Math.PI * 2)) / 2 * 100);
  const moonTransit = (12 + phase * 24) % 24; const moonDelta = ((hour - moonTransit + 36) % 24) - 12; const moonAltitude = Math.sin((1 - Math.min(1, Math.abs(moonDelta) / 12)) * Math.PI / 2) * 58 - 7;
  const skyTop = altitude < -8 ? "10244e" : altitude < 0 ? blendHex("10244e","6d78af",(altitude + 8) / 8) : altitude < 12 ? blendHex("6d78af","8bd6f3",altitude / 12) : blendHex("8bd6f3","55b9e5",noon);
  const skyBottom = altitude < -8 ? "294a7a" : altitude < 0 ? blendHex("294a7a","d3a9c6",(altitude + 8) / 8) : altitude < 12 ? blendHex("d3a9c6","dff7ff",altitude / 12) : "b6e7f6";
  const nightAlpha = Math.max(0, Math.min(1, (-altitude + 1) / 14));
  return { altitude, sunX:50 + Math.max(-42,Math.min(42, angle / 180 * 42)), sunY:Math.max(7,Math.min(108,72 - altitude * .86)), skyTop:`#${skyTop}`, skyBottom:`#${skyBottom}`, night:nightAlpha > .12, nightAlpha, sunVisibility:Math.max(0,Math.min(1,altitude / 4)), starOpacity:Math.max(0,Math.min(1,(-altitude - 2) / 15)), moonX:50 + Math.max(-36,Math.min(36,moonDelta * 3)), moonY:Math.max(12,Math.min(86,72 - moonAltitude * .82)), moonOpacity:nightAlpha * Math.max(0,Math.min(1,moonAltitude / 5)), illumination, moonShadow:`${phase <= .5 ? phase * 200 : -(1 - phase) * 200}%` };
}
function renderWorldCore(world, current, nextMission) {
  ...
  const celestial = solarProjection(now, Number(world.latitude) || 34.6937, Number(world.longitude) || 135.5023); const moonLight = celestial.illumination; const night = celestial.night;
  const stars = renderCelestialStars(now, Number(world.latitude) || 34.6937, Number(world.longitude) || 135.5023); const windDirection = Number(world.windDirection) || 0; const windStrength = Math.max(0, Math.min(18, Number(world.windSpeed) || 2)); const windRadians = (windDirection + 180) * Math.PI / 180;
...
function updateWorldCoreLive() {
  const globe = app.querySelector(".world-globe"); const core = app.querySelector(".world-core");
  if (!globe || !core) return;
  const world = state.world || {}; const celestial = solarProjection(new Date(), Number(world.latitude) || 34.6937, Number(world.longitude) || 135.5023);
  core.dataset.sky = celestial.night ? "night" : "day";
  globe.style.setProperty("--sun-x", `${celestial.sunX}%`); globe.style.setProperty("--sun-y", `${celestial.sunY}%`); globe.style.setProperty("--sun-visibility", celestial.sunVisibility);
  globe.style.setProperty("--sky-top", celestial.skyTop); globe.style.setProperty("--sky-bottom", celestial.skyBottom);
  globe.style.setProperty("--star-opacity", celestial.starOpacity); globe.style.setProperty("--night-alpha", celestial.nightAlpha);
  globe.style.setProperty("--moon-x", `${celestial.moonX}%`); globe.style.setProperty("--moon-y", `${celestial.moonY}%`); globe.style.setProperty("--moon-shadow", celestial.moonShadow); globe.style.setProperty("--moon-opacity", celestial.moonOpacity); globe.style.setProperty("--moon-brightness", .46 + celestial.illumination * .0054);
}
...
async function weather(lat, lon) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,precipitation,rain,weather_code,cloud_cover,wind_speed_10m,wind_direction_10m&timezone=auto`;
  const data = await fetch(url).then((r) => r.ok ? r.json() : Promise.reject());
...
function renderHome() {
  ...
  const now = new Date(); const p = state.player; const events = state.log.filter((e) => e.day === dateKey()).slice(0,3); const nav = navigatorBrief(w, p); const localScene = selectedHomeBgmScene(now, w); const activeTrack = { scene:localBgmTitle(localScene), role:`${localScene.label} · LOCAL LOOP`, tone:"night" }; const nextMission = upcomingCalendarEvent(); const greeting = navigatorGreeting(w, p); const current = currentPlayerState(w); const activeQuest = state.quests?.find((quest) => quest.status === "active"); const latestMessage = systemLog()[0]; const transition = state.pendingWorldTransition;
...
function renderPage(page, transitionDirection = "forward") {
  ...
  if (page === "sound") { content = `<section class="page-hero sound-hero night"><p class="eyebrow">local world audio</p><h1>SOUND</h1><p>この端末に登録したBGMを、REAL WORLDへ重ねる。</p><div class="sound-visual"><i></i><i></i><i></i><i></i><i></i></div></section>${renderSoundNowPlaying()}${renderLocalBgmSelector()}`; }
  ...
  app.querySelector("#search-home-area")?.addEventListener("click", async () => {
    ...
    try { const results = await searchAreaPlaces(query); if (!results.length) throw new Error("place not found"); Object.assign(areaSearchState(), { target:"home", query, name:"HOME BASE", type:"home", radius:80, results }); save(); renderPage("system"); } catch { alert("場所を見つけられませんでした。住所や駅名をもう少し詳しく入力してください。"); }
  });
  ...
  app.querySelectorAll("[data-local-bgm-scene]").forEach((button) => button.addEventListener("click", () => {
    const settings = homeBgmSettings(); settings.manualSceneId = button.dataset.localBgmScene === "auto" ? null : button.dataset.localBgmScene; save();
    syncHomeBgmScene(); if (settings.enabled && (!homeBgm || homeBgm.paused)) startHomeBgm(); renderPage("sound");
  }));
  ...
  if (page === "sound") bindLocalBgmPlayer();
  activateGlassPhysics();
}
