...
function renderWorldCore(world, current, nextMission) {
  ...
  const weather = String(world.weather || "").toUpperCase(); const cloudCover = Math.max(0, Math.min(100, Number(world.cloudCover) || 0)); const weatherMood = /THUNDER/.test(weather) ? "thunder" : /SNOW/.test(weather) ? "snow" : /RAIN|DRIZZLE|SHOWER/.test(weather) ? "rain" : cloudCover >= 55 || /CLOUD|OVERCAST|FOG/.test(weather) ? "cloud" : "clear";
...
function updateWorldCoreLive() {
  ...
  globe.style.setProperty("--sun-x", `${celestial.sunX}%`); globe.style.setProperty("--sun-y", `${celestial.sunY}%`);
  ...
  globe.style.setProperty("--moon-x", `${celestial.moonX}%`); globe.style.setProperty("--moon-y", `${celestial.moonY}%`); globe.style.setProperty("--moon-shadow", celestial.moonShadow); globe.style.setProperty("--moon-brightness", .46 + celestial.illumination * .0054);
}
...
async function weather(lat, lon) {
  ...
  return { weather: names[code] || "LOCAL CONDITIONS", temperature: Math.round(data.current?.temperature_2m), cloudCover:Math.round(data.current?.cloud_cover ?? 0) };
}
