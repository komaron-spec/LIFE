...
function soundtrackFor(world = state.world || {}) {
  ...
  const phaseValue = world.phase || phase(new Date().getHours());
  const weatherValue = world.weather || "";
  if (/RAIN|DRIZZLE|THUNDER/.test(weatherValue) && /NIGHT/.test(phaseValue)) return { scene:"RAIN AFTER DARK", type:"NIGHT / RAIN AMBIENT", copy:"雨音と夜の空気に重なる、静かな世界のBGM。", tone:"rain" };
  if (/RAIN|DRIZZLE|THUNDER/.test(weatherValue)) return { scene:"RAIN EVENT", type:"WEATHER / AMBIENT", copy:"雨の世界を観測するための、やわらかいアンビエント。", tone:"rain" };
  if (/HOME|HOUSE/.test(location) && /NIGHT|EVENING/.test(phaseValue)) return { scene:"HOME AFTER HOURS", type:"SAFE AREA / NIGHT", copy:"一日の終わりに、HOMEを安全地帯として感じる音。", tone:"home" };
  if (/HOME|HOUSE/.test(location)) return { scene:"HOME BASE", type:"SAFE AREA", copy:"自分の拠点で、次の行動を整えるための音。", tone:"home" };
  if (/NIGHT/.test(phaseValue)) return { scene:"NIGHT EXPLORER", type:"EXPLORATION / NIGHT", copy:"夜の街や帰り道に合う、少しだけ映画的な音。", tone:"night" };
  if (/MORNING/.test(phaseValue)) return { scene:"MORNING STARTUP", type:"DAWN / FOCUS", copy:"世界にログインし、ゆっくり動き始めるための音。", tone:"morning" };
  if (/AFTERNOON/.test(phaseValue)) return { scene:"DAYLIGHT FLOW", type:"DAY / MOMENTUM", copy:"日中の行動と探索を支える、軽いリズム。", tone:"day" };
  return { scene:"CITY EXPLORATION", type:"WORLD / EXPLORATION", copy:"いまいる世界を歩くための、あなたのサウンドトラック。", tone:"city" };
}
...
function renderHome() {
  ...
  const now = new Date(); const p = state.player; const events = state.log.filter((e) => e.day === dateKey()).slice(0,3); const nav = navigatorBrief(w, p); const soundtrack = soundtrackFor(w);
  ...
  <section class="card glass ${state.lastSyncEvents ? "events-new" : ""}"><div class="section-head"><p class="eyebrow">event / discovery</p><p class="event-engine-live"><i></i>${state.lastSyncEvents ? `${state.lastSyncEvents} NEW` : `AUTO · ${events.length}`}</p></div>${state.lastSyncEvents ? `<p class="new-events-banner">WORLD SYNCで新しい変化を検知しました</p>` : ""}${events.length ? events.map((e) => `<div class="log-entry"><p class="eyebrow">${esc(e.kind)} · ${esc(e.time)}</p><strong>${esc(e.title)}</strong><p>${esc(e.detail)}</p></div>`).join("") : `<div class="event"><i class="signal"></i><p>まだ新しいシグナルはありません。<br>SYNCすると、いまの世界を観測できます。</p></div>`}</section>
  ...
  <button class="card glass card-button now-playing-card" data-page="sound"><p class="eyebrow">now playing</p><div class="now"><i class="art ${soundtrack.tone}"></i><div><p><strong>${esc(state.soundtrack || soundtrack.scene)}</strong></p><small>${esc(state.soundtrack ? "PLAYER SELECTED" : soundtrack.type)}</small></div></div><p class="now-suggestion">WORLD SUGGESTION →</p></button>
...
function renderPage(page) {
  ...
  if (page === "sound") { const suggested = soundtrackFor(w); content = `<section class="page-hero sound-hero ${suggested.tone}"><p class="eyebrow">real world soundtrack</p><h1>SOUND</h1><p>いまの現実に、世界の音楽を重ねる。</p><div class="sound-visual"><i></i><i></i><i></i><i></i><i></i></div></section><section class="sound-recommendation glass ${suggested.tone}"><p class="eyebrow">world recommendation</p><h2>${esc(suggested.scene)}</h2><span>${esc(suggested.type)}</span><p>${esc(suggested.copy)}</p><button class="primary-action" id="use-recommendation">USE THIS SCENE</button></section><section class="detail-card glass"><p class="eyebrow">scene selection</p><div class="scene-grid">${[["HOME BASE","SAFE AREA"],["CITY EXPLORATION","EXPLORATION"],["NIGHT EXPLORER","NIGHT"],["RAIN EVENT","WEATHER"]].map(([scene,type]) => `<button class="scene-choice ${state.soundtrack === scene ? "selected" : ""}" data-scene="${scene}"><b>${scene}</b><span>${type}</span></button>`).join("")}</div><button class="subtle-action" id="auto-soundtrack">RETURN TO AUTO SUGGESTION</button><p class="ambience">時間帯・天気・場所を読み取り、毎回おすすめのBGMシーンを変えます。</p></section>`; }
  ...
  app.querySelector("#use-recommendation")?.addEventListener("click", () => { state.soundtrack = soundtrackFor(w).scene; save(); renderPage("sound"); });
  app.querySelector("#auto-soundtrack")?.addEventListener("click", () => { delete state.soundtrack; save(); renderPage("sound"); });
}
