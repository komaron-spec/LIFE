...
function renderWorldCore(world, current, nextMission) {
  ...
  const cloudOpacity = weatherMood === "clear" ? .035 + cloudCover * .0012 : weatherMood === "cloud" ? .3 + cloudCover * .0036 : .61;
...
async function weather(lat, lon) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,cloud_cover,wind_speed_10m,wind_direction_10m&timezone=auto`;
  const data = await fetch(url).then((r) => r.ok ? r.json() : Promise.reject());
  ...
  return { weather: names[code] || "LOCAL CONDITIONS", temperature: Math.round(data.current?.temperature_2m), cloudCover:Math.round(data.current?.cloud_cover ?? 0), windSpeed:Number(data.current?.wind_speed_10m ?? 0), windDirection:Number(data.current?.wind_direction_10m ?? 0) };
}
