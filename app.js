...
let systemAudio;
let idleTimer;
const feedbackSettings = () => (state.feedback ||= { sound:true, haptics:true });
...
app.addEventListener("click", (event) => {
  ...
  if (!control || control.matches("[data-page], [data-home], #sync, #page-sync, #system-sync, #toggle-sound, #toggle-haptics")) return;
...
function archivePrefectureState(prefecture) { const archive = archiveState(); if (!archive.prefectures[prefecture.id]) return "unknown"; return archive.places[`${prefecture.id}:landmark`] ? "explored" : "discovered"; }
function syncArchiveDiscovery(world, now) {
  ...
  if (prefecture && !archive.prefectures[prefecture.id]) { archive.prefectures[prefecture.id] = { firstSeen:now.toISOString(), visits:1 }; state.lastDiscovery = true; add("ARCHIVE", "NEW AREA DISCOVERED", `${prefecture.name} has been added to JAPAN ATLAS.`); }
...
const pageNav = (active) => `<nav class="page-nav glass" aria-label="LIFE SYSTEM navigation">${[["world","WORLD"],["archive","ATLAS"],["player","PLAYER"],["navigator","NAVI"],["log","LOG"],["sound","SOUND"],["system","SYSTEM"]].map(([name,label]) => `<button class="${active === name ? "is-active" : ""}" data-page="${name}" aria-current="${active === name ? "page" : "false"}">${label}</button>`).join("")}</nav>`;
...
function renderPage(page) {
  ...
  app.querySelector("#toggle-haptics")?.addEventListener("click", () => { const feedback = feedbackSettings(); feedback.haptics = feedback.haptics === false; save(); systemFeedback("confirm"); renderPage("system"); });
