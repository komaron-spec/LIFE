...
function renderPage(page) {
  ...
  if (page === "log") { const calendarEvents = state.calendar?.events || []; const calendarCount = state.calendar?.calendars?.length || 0; content = `<section class="page-hero"><p class="eyebrow">google calendar / timeline</p><h1>PLAYER LOG</h1><p>Google Calendarが記録する、あなたの世界のタイムライン。</p><div class="log-stat"><b>${calendarEvents.length}</b><span>EVENTS · ${calendarCount}<br>CALENDARS</span></div></section>${calendarEvents.length ? `<section class="detail-card glass full-log calendar-timeline"><div class="section-head"><p class="eyebrow">world schedule</p><span>ALL CALENDARS</span></div>${[...calendarEvents].reverse().map((event) => { const [label,copy,tone] = calendarLogState(event); return `<article class="calendar-entry ${tone}"><p class="eyebrow">${esc(calendarLogDate(event))} · ${esc(calendarTime(event))} · <span>${label}</span></p><strong>${esc(event.title)}</strong><p><b class="calendar-source">${esc(event.calendarName || "PRIMARY CALENDAR")}</b>${event.location ? ` · ${esc(event.location)}` : ` · ${copy}`}</p></article>`; }).join("")}</section>` : `<section class="detail-card glass calendar-link"><p class="eyebrow">calendar not connected</p><strong>予定をLOGに読み込もう</strong><p>手入力は必要ありません。Google Calendarを連携して更新すると、予定がPLAYER LOGになります。</p><button class="primary-action" data-page="system">GO TO CALENDAR SETUP</button></section>`}`; }
